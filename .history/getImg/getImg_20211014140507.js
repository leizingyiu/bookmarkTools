...
/* 
"Last modified": "2021/10/14 14:05:05"
*/
(function () {
            ...
            const result = [...document.querySelectorAll('a')].map(async function (a) {
                const p = await new Promise((resolve, reject) => {
                    fetch(a.href)
                        ...
                        .then(t => t.match(/<script type="text\/javascript">window\.__additionalDataLoaded.*<\/script>/)[0].match(/(?<=>).*(?=<\/)/)[0].replace(/window\.__additionalDataLoaded\(.*\)$, '').replace(/;$/, ''))
                        .then(t => JSON.parse(t))
