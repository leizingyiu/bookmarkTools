...
/* 
"Last modified": "2021/10/14 13:01:50"
*/
(function () {
            ...
            const result = [...document.querySelectorAll('a')].map(async function (a) {
                const p = await new Promise((resolve, reject) => {
                    fetch(a.href)
                        ...
                        .then(t => t.match(/<script type="text\/javascript">window\._sharedData = .*<\/script>/)[0].match(/(?<=>).*(?=<\/)/)[0].replace(/window\._sharedData/, 'return result'))
